...
const AddTreatment = () => {
  ...
  const {
    ...
    userId ? () => ClientService.getAllClientsByUser(userId) : null,
    [userId]
  );
  ...
  const {
    ...
    userId ? () => UserService.getAllSuppliesByUser(userId) : null,
    [userId]
  );
  ...
  const handleAddSupply = () => {
    ...
    if (existingSupply) {
      toast.error(
        "This supply is already selected. Please modify the existing entry."
      );
      return;
    }
    ...
    setSelectedSupplies((prev) => [
      ...prev,
      {
        suppliesId: selectedSupplyId,
        supplyName: selectedSupply.brandName,
        quantity,
      },
    ]);
  ...
  const handleRemoveSupply = (supplyId) => {
    setSelectedSupplies((prev) =>
      prev.filter((s) => s.suppliesId !== supplyId)
    );
  };
